...
export default function PostsClient({
    ...
    // igPosts comes from server, but we can also manage it locally if needed,
    // though usually we just rely on props + router.refresh()
    const igPosts = initialIgPosts

    // We can use a simpler loading state just for actions
    ...
    const handleRefresh = () => {
        setRefreshing(true)
        router.refresh()
        // Simple timeout to clear spinner, since router.refresh is async but doesn't return promise
    ...
    const handleSyncToGBP = async (post: Post) => {
        ...
        try {
            if (!googleToken) {
                // If token not passed from server, maybe user not linked OR token missing
                // We can't easily fetch it here if not passed.
                // Assuming passed. If not, error.
                throw new Error('Google連携トークンが見つかりません。連携設定を確認してください。')
            }

            // 1. アカウント取得
            const accounts = await fetchGBPAccounts(googleToken)
            const accountId = accounts.accounts?.[0]?.name
            if (!accountId) throw new Error('Googleビジネスプロフィールアカウントが見つかりません')

            // 2. 店舗取得 (最初の1店舗)
            const locations = await fetchGBPLocations(googleToken, accountId)
            const locationId = locations.locations?.[0]?.name
            if (!locationId) throw new Error('店舗情報が見つかりません')

            // 3. 投稿作成
            await createGBPPost(googleToken, locationId, post.content, post.imageUrl)
            ...
            if (msg.includes('401') || msg.includes('403')) {
                msg = 'Google認証に失敗しました。連携し直してください。'
                    ...
                    <TabsContent value="all" className="space-y-4 mt-4">
                        {allPosts.map(post => (
                            <PostCard
                                key={post.id}
                                post={post}
                                onSync={post.platform === 'instagram' ? () => handleSyncToGBP(post) : undefined}
